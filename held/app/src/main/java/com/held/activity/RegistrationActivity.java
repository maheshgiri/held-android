...
public class RegistrationActivity extends ParentActivity implements View.OnClickListener, NetworkStateReceiver.OnNetworkChangeListener {
    ...
    private void validateInput() {
        ...
        if (!TextUtils.isEmpty(mUserNameEdt.getText().toString().trim())) {
            if (mUserNameEdt.getText().toString().trim().length() < 6) {
                mUserNameEdt.setError("Please enter minimum 6 chars");
    ...
    private void callCreateUserApi() {
        HeldService.getService().createUser(mCountryCode + mPhoneNoEdt.getText().toString().trim(), mUserNameEdt.getText().toString().trim(), new Callback<CreateUserResponse>() {
            @Override
